...
// Thiết lập chỉ số slide hiện tại
var currentSlide = 0;
...
function showSlide(n) {
  ...
  // Xác định chỉ số slide kế tiếp
  currentSlide = (n + slides.length) % slides.length;

  // Hiển thị slide kế tiếp
  slides[currentSlide].classList.add("active");
}
...
// Thiết lập thời gian chuyển đổi slide (đơn vị: milliseconds)
var slideInterval = setInterval(autoSlide, 3000);

// Dừng chuyển đổi slide khi rê chuột vào slideshow
var slider = document.querySelector(".home-slider");
slider.addEventListener("mouseover", function () {
  clearInterval(slideInterval);
});

// Tiếp tục chuyển đổi slide khi rời chuột khỏi slideshow
...
function detailProduct(index) {
  ...
  let modalHtml = `
      <button class="modal-close close-popup" onclick="closeModal()">
